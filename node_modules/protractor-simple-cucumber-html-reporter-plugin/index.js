...
const simpleCucumberHTLMReporter = require("simple-cucumber-html-reporter");
...
const PLUGIN_CONFIG = {
    /**
     * simple-cucumber-html-reporter specific options
     */
    disableLog: false,
    ...
    /**
     * simple-cucumber-html-reporter-plugin specific options
     */
 ...
 */
function setup() {
    return browser.getProcessedConfig().then((configuration) => {
        ...
        if (IS_JSON_FORMAT) {
            ...
            if (PLUGIN_CONFIG.jsonOutputPath === JSON_OUTPUT_FOLDER) {
                PLUGIN_CONFIG.jsonOutputPath = path.join(
             ...
             */
            PLUGIN_CONFIG.uniqueReportFileName =
                (Array.isArray(configuration.multiCapabilities) &&
            ...
            PID_INSTANCE_DATA = {
                ...
                metadata: Object.assign(
                    metadata,
                    configuration.capabilities[PLUGIN_CONFIG.metadataKey] || {}
                ),
            };
...
function postResults() {
    if (IS_JSON_FORMAT) {
      ...
      PLUGIN_CONFIG.cucumberReportName
        ...
        if (
            (!PLUGIN_CONFIG.uniqueReportFileName && fs.existsSync(reportPath)) ||
            (PLUGIN_CONFIG.uniqueReportFileName && fs.existsSync(pidReportPath))
        ) {
            const currentReportPath = PLUGIN_CONFIG.uniqueReportFileName ?
                pidReportPath :
                reportPath;
            ...
            currentReport.map((singleReport) => {
                ...
                if (PLUGIN_CONFIG.removeExistingJsonReportFile) {
                    fs.readdirSync(PLUGIN_CONFIG.jsonOutputPath)
                        ...
                        .forEach((file) =>
                            fs.removeSync(path.resolve(PLUGIN_CONFIG.jsonOutputPath, file))
                        );
                }
                ...
                fs.writeJsonSync(
                    filePath,
                    JSON.parse(`[${JSON.stringify(singleReport)}]`), { spaces: 2 }
                );
            });
            ...
            if (
                (PLUGIN_CONFIG.customData || PLUGIN_CONFIG.reportName) &&
                !PLUGIN_CONFIG.automaticallyGenerateReport
            ) {
                console.warn(`
===========================================================================
    YOU ADDED A CUSTOM reportName AND OR ADDED A custom Data-object
...
===========================================================================`);
            ...
            if (PLUGIN_CONFIG.automaticallyGenerateReport) {
                ...
                if (PLUGIN_CONFIG.reportPath === REPORT_FOLDER) {
                    PLUGIN_CONFIG.reportPath = path.join(
                        PLUGIN_CONFIG.cucumberResultsPath,
                        REPORT_FOLDER
                    );
                }

                const simpleCucumberHTLMReporterConfig = {
                ...
                if (PLUGIN_CONFIG.reportName) {
                    simpleCucumberHTLMReporterConfig.reportName =
                        PLUGIN_CONFIG.reportName;
                }
                ...
                if (PLUGIN_CONFIG.customData) {
                    simpleCucumberHTLMReporterConfig.customData =
                        PLUGIN_CONFIG.customData;
                }

                simpleCucumberHTLMReporter.generate(simpleCucumberHTLMReporterConfig);
            }
        } else {
            console.warn(
